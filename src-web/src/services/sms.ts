...
export const getSMSListByNumber = (
  page: number,
  number: string
): Promise<any> => {
  return fetch(
    `/api/get_messages?token=${getToken()}&page=${page}&number=${encodeURIComponent(
      number
    )}`
  )
    .then((resp) => resp.json())
...
export const clearLog = (): Promise<any> => {
  return fetch(`/api/clear_log?token=${getToken()}`)
    .then((resp) => resp.json())
    .catch((e) => {
      console.error(e);
    });
